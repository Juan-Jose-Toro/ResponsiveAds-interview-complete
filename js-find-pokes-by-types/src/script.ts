...
const type = "grass";
const answer1 = pokes.filter((x) => x.types.includes(type));
...
const answer2Or = pokes.filter(curPoke => {
  let res = curPoke.types.filter(
    curType => types.includes(curType)
  );
  return res.length >= 1;
});
const answer2And = pokes.filter(curPoke => {
  let res = curPoke.types.filter(curType => types.includes(curType))
  return res.length == types.length
})
